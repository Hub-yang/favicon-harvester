...
Chrome 商店「隐私声明」页要求填一个可公开访问的隐私政策 URL。仓库保持 private，所以用一个**公开 Gist**单独承载：

1. 浏览器打开 <https://gist.github.com/>。如果右上角显示的是「Sign in」，先登录你的 GitHub 账号（用登录仓库 `Hub-yang` 的那个账号即可，Gist 和仓库私有性互不影响）。
2. 登录后页面会显示一个新建 gist 的表单，从上到下依次是：一个单行的「描述」输入框、一个「文件名」输入框、一个大文本框（写内容）。
3. 「描述」输入框（选填，写不写都不影响可用性）可以填：
   ```
   图标提取器 Chrome 插件隐私政策
   ```
4. 「文件名」输入框（`Filename including extension...`）填：
   ```
   PRIVACY.md
   ```
   文件名必须带 `.md` 后缀，GitHub 才会按 Markdown 渲染成带格式的页面，不然打开是纯文本。
5. 下方大文本框里粘贴这段内容：
...
6. 表单最下方有一个绿色按钮，右侧带一个向下箭头（▾）的下拉选择器，可以在 **Create public gist** 和 **Create secret gist** 之间切换——**必须选 Create public gist**（如果按钮当前显示的是 "Create secret gist"，点一下箭头切换过去再点按钮）。secret gist 虽然也有链接，但不会被搜索到、审核方可能判定为"未公开发布"而打回，所以不能选它。
7. 点击 **Create public gist** 提交。创建成功后会自动跳转到这个 gist 的详情页，浏览器地址栏的 URL 就是形如 `https://gist.github.com/<你的用户名>/<一串哈希>` 的样子。
8. 复制这个地址栏 URL——这就是后面「隐私声明」页要填的隐私政策 URL（1.4 节步骤 5）。可以先在无痕窗口/未登录状态下打开这个链接确认一下，能正常看到渲染后的隐私政策内容即为有效。
