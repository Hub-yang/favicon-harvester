...
找到的图标会去重后按尺寸从大到小列在面板里，每一个都能预览实际效果和尺寸。你可以挑需要的那些单独下载，也可以把它们打包成一个 ZIP 一次拿走，包里附了一份清单，记着每个图标是从哪里找到的、多大、什么格式。单个图标还能直接复制走——复制地址、复制图片本身、复制 Data URI，或者复制一段现成的 link 标签。下载得到的是网站上的原始文件，插件不做任何转码或压缩。
