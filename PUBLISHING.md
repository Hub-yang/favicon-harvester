...
1. 打开 <https://gist.github.com/>（需要登录 GitHub）。
2. 新建 gist，文件名填 `PRIVACY.md`，内容粘贴下面这段文本：
   ...
   如有疑问，请通过 `[联系邮箱，发布前请自行填写]` 联系。
   ```

   > 隐私政策里的联系方式占位符（`[联系邮箱...]`）需要你自己填一个愿意公开的邮箱，我没有替你决定要不要公开哪个邮箱。

3. 点击页面下方 **Create public gist**（不要选 secret gist，secret gist 虽然有链接但不会出现在公开索引里，仍算"未公开"，Chrome 审核可能判定无法访问）。
4. 创建后复制浏览器地址栏的 URL（形如 `https://gist.github.com/<你的用户名>/<hash>`），这个 URL 就是后面「隐私声明」页要填的隐私政策 URL。
