...
- **原样下载**。下载的就是网站上那个文件本身，不转码、不压缩、不改尺寸。文件名按 `域名-来源-尺寸.扩展名` 自动生成，统一落在下载目录的 `favicon-harvester/<域名>/` 下，不会弄乱下载根目录，一次点击也能把找到的全部拿走。
- **四种复制方式**。展开卡片上的「复制」，可以取链接、图片本身、Data URI，或者一段现成的 `<link rel="icon">` 标签直接粘进 `<head>`。复制图片只对 PNG 开放——其他格式要塞进系统剪贴板就得转码，那跟「原样」这条自相矛盾，所以宁可置灰并说明原因。
- **中英双语**。界面文案和扩展名各有一套简体中文与英文，按浏览器的界面语言自动切换，插件里不需要设置。
...
打开任意网页 → 点工具栏上的插件图标 → 面板里挑图标单个下载，或一键打包成一个 ZIP 拿走（包内附一份 `icons.json`，记录每个图标的来源、尺寸与格式）。单个图标也可以复制成链接、图片、Data URI 或现成的 `<link>` 标签。
